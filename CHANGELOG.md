...
- **PKCS#11 reference dataset (demo).** A **Load PKCS#11 reference data** action (shown on demo profiles, in the Coverage tab) seeds a complete, internally-consistent end-to-end example for **C_Sign, C_GenerateKeyPair, C_Verify**: functional + customer requirements, test cases, test↔requirement links, executions with run status, and the full coverage layer (canonical features → parameter models → value→test mappings → two versions → member version-locks → change requests with per-customer decisions). Every mapped test exists and is browsable, so a value in the Coverage matrix is the same test you can open in Browse — a ready-made reference for the whole data relationship. Idempotent.
