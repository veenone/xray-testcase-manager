...
func TestNewBackendRouting(t *testing.T) {
	if _, ok := newBackend("kiwi", "https://kiwi.example.com", "user:pass").(*kiwi.Adapter); !ok {
		t.Error(`newBackend("kiwi", real URL) did not return a *kiwi.Adapter`)
	}

	if _, ok := newBackend("xray", "https://jira.example.com", "tok").(*xray.Adapter); !ok {
		t.Error(`newBackend("xray", real URL) did not return a *xray.Adapter`)
	}
	if _, ok := newBackend("", "https://jira.example.com", "tok").(*xray.Adapter); !ok {
		t.Error(`newBackend("", real URL) did not return a *xray.Adapter`)
	}

	if _, ok := newBackend("", "kiwi-demo", "tok").(*kiwi.Adapter); !ok {
		t.Error(`newBackend("", "kiwi-demo") did not return a *kiwi.Adapter`)
	}
	if _, ok := newBackend("xray", "kiwi-demo", "tok").(*kiwi.Adapter); !ok {
